...
public class Main {
    public static void main(String[] args) {
        ...
        в неструктурированном формате. Бухгалтерия попросила написать программу, в которой  можно работать с Ф. И. О. сотрудников.
        ...
        System.out.println("Данные Ф. И. О. сотрудника для заполнения отчета — ".concat(fullName.toUpperCase()));
    ...
    public static String InputString(String nameText) {
        Scanner getFIO = new Scanner(System.in);
        boolean allLetters = false;
        String inputDate = "";
        while (!allLetters) {
            System.out.print(nameText);
            inputDate = getFIO.next();
            allLetters = inputDate.chars().allMatch(Character::isLetter);
            if (!allLetters) {
                System.out.println("Допускается только ввод символов");
            }
        }
        return inputDate.substring(0, 1).toUpperCase()+inputDate.substring(1).toLowerCase();
    }
}
